...
PATIENCE = 15            # To stop model early, if it is not learning anything, or anymore
...
def train_fn(train_loader, model, optimizer, loss_fn, accum_steps=4):
    ...
    Parameters:
        ...
        optimizer (Optimizer): The optimizer for updating model weights. YOLOv1 uses SGD with momentum.
...
def main():
    ...
    # Set up the optimizer using the model's parameters and the defined learning rate and weight decay
    # Nesterov is not mentioned in the original paper only for experiment purposes, it accelerates the optimization
